...
5) Optionally hold heel-to-toe TVD around a target (e.g. 2500 m) as much as possible,
   while still staying inside the WELL_GUIDE window.
...
# Keep lateral/heel-to-toe section near a desired TVD as much as possible.
enable_tvd_hold = True
target_tvd = 2500.0            # desired TVD for heel-to-toe section
tvd_hold_weight = 0.85         # 0=no hold, 1=aggressive hold to target TVD
heel_inclination_deg = 80.0    # inclination threshold defining heel-to-toe section
...
def inclination_deg_from_dir(direction: np.ndarray) -> float:
    """Inclination in degrees from vertical axis."""
    d = normalize(direction)
    return float(np.degrees(np.arccos(np.clip(abs(d[2]), -1.0, 1.0))))
...
def build_guide_control_points(
    ...
    for i in range(len(bins) - 1):
        ...
        zmin = float(np.min(chunk[:, 2]))
        zmax = float(np.max(chunk[:, 2]))
        z = zmin + np.clip(bias, 0.0, 1.0) * (zmax - zmin)

        cps.append([x, y, z])
        cps_proj.append(float(np.mean(sorted_proj[mask])))
        cp_zmin.append(zmin)
        cp_zmax.append(zmax)

    control_points = np.array(cps, dtype=float)
    cp_proj = np.array(cps_proj, dtype=float)
    cp_zmin = np.array(cp_zmin, dtype=float)
    cp_zmax = np.array(cp_zmax, dtype=float)

    if len(control_points) < 2:
        raise RuntimeError("Could not build enough guide control points.")

    return control_points, major_axis, cp_proj, cp_zmin, cp_zmax
...
def plan_sidetrack(
    ...
    forward_zmin = cp_zmin[forward_mask]
    forward_zmax = cp_zmax[forward_mask]
    ...
    for _ in range(n_steps):
        ...
        target = forward_cp[look_ahead_idx].copy()

        if enable_tvd_hold and inclination_deg_from_dir(current_dir) >= heel_inclination_deg:
            zmin = forward_zmin[look_ahead_idx]
            zmax = forward_zmax[look_ahead_idx]
            z_tvd_clipped = np.clip(target_tvd, min(zmin, zmax), max(zmin, zmax))
            target[2] = (1.0 - tvd_hold_weight) * target[2] + tvd_hold_weight * z_tvd_clipped
...
control_points, major_axis, cp_proj, cp_zmin, cp_zmax = build_guide_control_points(
    selected_xyz,
    num_bins,
    structure_bias,
)
trajectory = plan_sidetrack(existing_xyz, control_points, major_axis, cp_proj, cp_zmin, cp_zmax)
...
print(
    "TVD hold:",
    f"enabled={enable_tvd_hold},",
    f"target_tvd={target_tvd:.1f} m,",
    f"weight={tvd_hold_weight:.2f},",
    f"heel_inclination_deg={heel_inclination_deg:.1f}",
)
